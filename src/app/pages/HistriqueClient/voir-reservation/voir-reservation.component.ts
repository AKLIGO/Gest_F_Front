...
export class VoirReservationComponent implements OnInit{
  ...
  applyFilters(): void {
    ...
    // Filtre par recherche (appartement, ID)
    if (this.searchQuery.trim()) {
      const query = this.searchQuery.toLowerCase();
      filtered = filtered.filter(res => 
        res.appartementNom?.toLowerCase().includes(query) ||
  ...
  sortReservations(reservations: ReservationResponseDTO[]): void {
    switch(this.sortBy) {
      ...
      case 'date-asc':
        ...
        break;
      case 'nom':
        reservations.sort((a, b) => a.appartementNom.localeCompare(b.appartementNom));
        break;
    }
  }
  ...
  loadCancellationInfos(): void {
    this.reservations.forEach(reservation => {
      this.reservationService.getCancellationInfo(reservation.id).subscribe({
        next: (info) => {
  ...
  cancelReservation(reservationId: number): void {
    ...
    this.reservationService.cancelReservation(reservationId).subscribe({
      next: (updatedReservation) => {
        console.log('Réservation annulée avec succès', updatedReservation);
